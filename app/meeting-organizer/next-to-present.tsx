...
export default function NextToPresent() {
    ...
    const [yetToPresentIndexes, setYetToPresentIndexes] = useState<number[]>(
        Object.keys(meetingParticipants).map(Number)
    );
    ...
    const spinTheWheel = () => {
        ...
        // Flash through random names for 2 seconds
        let elapsed = 0;
        const flashDuration = 2000; // 2 seconds total
        const intervalTime = 150; // Flash every 150ms

        const interval = setInterval(() => {
            const randomIndex =
                yetToPresentIndexes[
                    Math.floor(Math.random() * yetToPresentIndexes.length)
                ];
            setPresentingIndex(randomIndex);
            elapsed += intervalTime;

            if (elapsed >= flashDuration) {
                clearInterval(interval);
                // Select final random index
                const finalIndex =
                    yetToPresentIndexes[
                        Math.floor(Math.random() * yetToPresentIndexes.length)
                    ];
                setPresentingIndex(finalIndex);
                setYetToPresentIndexes((prevIndexes) =>
                    prevIndexes.filter((index) => index !== finalIndex)
                );
                setSpinning(false);
            }
        }, intervalTime);
    ...
    const saveNotes = (
        ...
        if (type === "wins")
            meetingParticipants[index].wins.push({
                ticketId: ticketId,
                notes: notes,
            });
        if (type === "needsPeerReview")
            meetingParticipants[index].needsPeerReview.push({
                ticketId: ticketId,
                notes: notes,
            });
        if (type === "blockers")
            meetingParticipants[index].blockers.push({
                ticketId: ticketId,
                notes: notes,
            });
    };
    ...
    return (
        <>
            <div className="flex flex-col justify-center gap-4">
                {presentedIndexes.length ==
                Object.keys(meetingParticipants).length ? null : (
                    <Button
                ...
                <div className="flex flex-row gap-4">
                    <div className="flex flex-col h-min w-1/3 gap-2 items-center rounded-2xl border-2 border-solid border-grey-500 bg-background-variant shadow-md p-4">
                        ...
                        <ul className="flex flex-col gap-2 w-3/4">
                            {yetToPresentIndexes.map((index) => (
                                <li
                                    key={index}
                                    className="group cursor-pointer flex items-center bg-white rounded-2xl p-2 px-4 justify-between"
                                    onClick={() => {
                                        cuePresentingIndex(index);
                                    }}
                                >
                    ...
                    <div className="flex flex-col h-min w-1/3 border-2 border-solid rounded-2xl border-grey-500 items-center shadow-md bg-background-variant p-4">
                        {presentedIndexes.length ==
                        Object.keys(meetingParticipants).length ? (
                            ...
                            <div className="flex flex-col min-h-1/2 gap-3 items-center">
                                ...
                                {(presentedIndexes.length ==
                                    Object.keys(meetingParticipants).length &&
                                    presentedIndexes.length > 0) ||
                                presentingIndex < 0 ? null : (
                                    <div className="h-1/2">
                                        <SubmitWorkerNotes
                                            saveNotes={saveNotes}
                    ...
                    <div className="flex flex-col items-center h-min w-1/3 rounded-2xl border-2 border-solid border-grey-500 shadow-md bg-background-variant p-4 gap-2">
                        <div className="flex flex-row gap-4 items-center">
                            ...
                            <FiCopy
                                className={`cursor-pointer ${copied || presentedIndexes.length == 0 ? "hidden" : "inline-block"}`}
                                onClick={() =>
                                    copyPresentedSummaryToClipboard()
                                }
                            />
                            <LuCopyCheck
                                className={`text-green-500 ${copied ? "inline-block" : "hidden"}`}
                            />
                        </div>
                        <ul className="flex flex-col gap-2 w-3/4">
